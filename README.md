...
This was never designed as a starter kit. It's a personal daily driver built to improve my workflow first and foremost — practical, with some nice touches, but not trying to be a unixporn showcase. If you can derive any value from this on top of that, then that's a plus in my book.
...
My external tools: mako, fuzzel, grim, slurp, swappy, cliphist, wf-recorder, wl-mirror.

The entire config is 205 lines and reloads live on `Super+r` since it's symlinked. `eDP-1` is hardcoded as the monitor output — check yours with `mangoctl outputs` and change it.
...
Also included: render-markdown for inline markdown and LaTeX preview in buffers, undotree for change history, vim-visual-multi for multi-cursor editing, and which-key for keybind discovery. See the full plugin list in `modules/home/neovim.nix`.
